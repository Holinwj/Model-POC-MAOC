...
data = read_excel("./2_pool_model/data_2pools.xlsx")
...
data_C = read_excel("./2_pool_model/data_C_2pools.xlsx")
pom = data_C[,c(1:3)]
...
random_inipars <- function(ranges) {
  c(runif(1, ranges$k1[1],  ranges$k1[2]),
...
results <- future_map(
  ...
  function(i) {
    ...
    out <- try({
      fit_i <- twopsFit(rt, initial, inipars_try)
      list(
        fit     = fit_i,
...
best_models <- {
  ...
  top50 <- results[order(AIC_values)][1:5]
...
# fit the two pool series model with known initial C values 
M1=twopsFit(rt, initial, inipars=c(0.18810329, 0.03180069, 0.77196821)) # fit resp
t1=M1$SoilRmodel@times                    
C1=getC(M1$SoilRmodel)                
M1$FMEmodel$par                     
M1$FMEmodel$ms
...
best_models <- {
  AIC_values <- map_dbl(results, "AIC")
  top50 <- results[order(AIC_values)][1:15]
...
# fit the two pool series model with known initial C values 
M1=twopsFit(rt, initial, inipars=c(0.002963183, 0.138234048, 0.275904860)) #Fit resp 
t1=M1$SoilRmodel@times    
C1=getC(M1$SoilRmodel)              
M1$FMEmodel$par
M1$FMEmodel$ms
...
outputs <- data.frame(
  ...
  a21 = c(
    C05_resp$FMEmodel$par[3]*C05_resp$FMEmodel$par[1],
    C05_pools$FMEmodel$par[3]*C05_pools$FMEmodel$par[1],
    L05_resp$FMEmodel$par[3]*L05_resp$FMEmodel$par[1],
    L05_pools$FMEmodel$par[3]*L05_pools$FMEmodel$par[1]
  ),
